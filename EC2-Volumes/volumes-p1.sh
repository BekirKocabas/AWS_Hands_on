...
lsblk --> blok devicelari, diskleri listeliyor, xvda1 cikar o part
df -h --> file systeme iliskin bilgileri verir. 
# Check file system of the root volume's partition.
sudo file -s /dev/xvda1 -> hangi file sistemi oldugunu görürüz. XFS amazaon linuxde
...
sudo growpart /dev/xvda 1 -> partitioni büyütmek icin kullanilir. 1 ayri dikkat.
# Resize the xfs file system on the extended partition to cover all available space.
sudo xfs_growfs /dev/xvda1 -> bu da file sistemi büyütür
# List block devices (lsblk) and file system disk space usage (df) of the instance again.
