...
```text
Bucket name                 : myfirstbucket-YOUR_NAME_HERE-01(students use their own student name number)
Region                      : N.Virginia
...
- Create a new bucket for static website with name of `pet.bekir.static.web.hosting` and with following properties

```text
Bucket name                 : pet.bekir.static.web.hosting
Region                      : N.Virginia
...
- Click the S3 bucket `pet.bekir.static.web.hosting` and upload following files.
...
- Show static website hosting settings from properties of `pet.bekir.static.web.hosting` bucket.
...
- Create folder called `kitten` under the bucket named `pet.bekir.static.web.hosting`.
...
- Create a new bucket named `pet.bekir.versioning` with following properties.

```text
Bucket name                 : pet.bekir.versioning
Region                      : N.Virginia
Object Ownership            : ACLs disabled (recommended)****
...
- Click the S3 bucket `pet.bekir.versioning` and upload following files.
...
- Show static website hosting settings from properties of `pet.bekir.versioning` bucket.
```
PROPERTIES>>>>> STATIC WEBSITE HOSTING
```
- Click static web hosting and put check mark to `Use this bucket to host a website` and enter `index.html` as default file.
...
- Show versioning of the files right under the bucket `pet.bekir.versioning`
