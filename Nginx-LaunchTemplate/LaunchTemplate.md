...
18. Select `MyTemplate` ---> `Actions` ---> `Launch Instance from Template`
...
23. Select template named `MyTemplate` ---> `Actions` ---> `Modify template (Create New Version)`
...
28. Go to `Launch Template` Menu and click on `MyTemplate`.
...
35. Select template named `MyTemplate` ---> `Actions` ---> 'Modify template (Create New Version)'.
